...
async function handler(ctx) {
    ...
    if (ctx.req.param('routeParams')) {
        if (ctx.req.param('routeParams') === '1' || ctx.req.param('routeParams') === '0') {
            ...
            showLargeCommentImg = fallback(undefined, queryToBoolean(routeParams.showLargeCommentImg), false) ? '1' : '0';
        }
    }
        ...
        cards
            ...
            .map(async (item) => {
                ...
                if (displayComments === '1') {
                    description = await weiboUtils.formatComments(ctx, description, item.mblog, showLargeCommentImg === '1');
                }
